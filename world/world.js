...
(async function () {
  ...
  if ($("#set-logout")) $("#set-logout").hidden = !onVercel;
  if ($("#set-logout")) $("#set-logout").addEventListener("click", async () => { try { await fetch("/api/login", { method: "DELETE", credentials: "same-origin" }); } catch {} location.href = "/login"; });
